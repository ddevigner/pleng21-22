...
public class SemanticFunctions {
	...
	public void AddVar(Attributes var, Token t, Token i, Types type) {
		try {
			...
			se.detection(e, t);
			var.type = var.baseType = Types.UNDEFINED;
		}
	}
	...
	private void evaluateMultipleAssignationOrder(Attributes at) throws BadMultipleAssignationOrderException {
		if (at.ass.size() != at.exp.size()) throw new BadMultipleAssignationOrderException();
	}

	public void EvaluateMultipleAssignation(Attributes at, Token t) {
		try {
			evaluateMultipleAssignationOrder(at);
			try {
				for (int i = 0; i < at.ass.size(); i++) {
					evaluateAssignation(at.ass.get(i).baseType, at.exp.get(i).baseType);
					at.code.addBlock(at.ass.get(i).code);
					at.code.addBlock(at.exp.get(i).code);
					at.code.addInst(OpCode.ASG);
				}
			} catch (MismatchedTypesException e) {
				se.detection(e, t);
			}
		} catch (BadMultipleAssignationOrderException e) {
			se.detection(e, t);
		}
	}
